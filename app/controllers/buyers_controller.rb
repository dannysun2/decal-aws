class BuyersController < ApplicationController
  ...
  def watch_user
    Watchlist.create user_id: current_user.id, buyer_id: params[:user_id]
    redirect_to :back
  end

  def unwatch_user
    Watchlist.find_by(user_id: current_user.id, buyer_id: params[:buyer_id]).destroy
    redirect_to :back
  end
  ...
  private
    ...
    def buyer_params
      params.require(:buyer).permit(:email, :name, :phone)
    end
end
